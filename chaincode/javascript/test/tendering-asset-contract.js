...
describe('TenderingAssetContract', () => {
    ...
    beforeEach(() => {
        contract = new TenderingAssetContract();
        ctx = new TestContext();
    ...
    describe('#tenderingAssetExists', () => {

        it('should return true for a private asset that exists', async () => {
            await contract.tenderingAssetExists(ctx, '001').should.eventually.be.true;
        });

        it('should return false for a private asset that does not exist', async () => {
            await contract.tenderingAssetExists(ctx, '002').should.eventually.be.false;
        });

    });

    describe('#createTenderingAsset', () => {

        it('should throw an error for a private asset that already exists', async () => {
            await contract.createTenderingAsset(ctx, '001').should.be.rejectedWith('The asset tendering asset 001 already exists');
        });
        ...
        it('should throw an error if transient data is not provided when creating private asset', async () => {
            ...
            await contract.createTenderingAsset(ctx, '002').should.be.rejectedWith('The privateValue key was not specified in transient data. Please try again.');
        });
        ...
        it('should create a private asset if transient data key is privateValue', async () => {
            ...
            ctx.stub.getTransient.returns(transientMap);
            await contract.createTenderingAsset(ctx, '002');
            ctx.stub.putPrivateData.should.have.been.calledOnceWithExactly(collectionName, '002', Buffer.from('{"privateValue":"1500"}'));
        });

    });

    describe('#readTenderingAsset', () => {

        it('should throw an error for my private asset that does not exist', async () => {
            await contract.readTenderingAsset(ctx, '003').should.be.rejectedWith('The asset tendering asset 003 does not exist');
        });

        it('should return my private asset', async () => {
            await contract.readTenderingAsset(ctx, '001').should.eventually.deep.equal({ privateValue: '150' });
            ctx.stub.getPrivateData.should.have.been.calledWithExactly(collectionName, '001');
        });

    });

    describe('#updateTenderingAsset', () => {

        it('should throw an error for my private asset that does not exist', async () => {
            await contract.updateTenderingAsset(ctx, '003').should.be.rejectedWith('The asset tendering asset 003 does not exist');
        });

        it('should throw an error if transient data is not provided when updating private asset', async () => {
            let transientMap = new Map();
            ctx.stub.getTransient.returns(transientMap);
            await contract.updateTenderingAsset(ctx, '001').should.be.rejectedWith('The privateValue key was not specified in transient data. Please try again.');
        });
        ...
        it('should update my private asset if transient data key is privateValue', async () => {
            ...
            ctx.stub.getTransient.returns(transientMap);
            await contract.updateTenderingAsset(ctx, '001');
    ...
    describe('#deleteTenderingAsset', () => {

        it('should throw an error for my private asset that does not exist', async () => {
            await contract.deleteTenderingAsset(ctx, '003').should.be.rejectedWith('The asset tendering asset 003 does not exist');
        });

        it('should delete my private asset', async () => {
            await contract.deleteTenderingAsset(ctx, '001');
            ctx.stub.deletePrivateData.should.have.been.calledOnceWithExactly(collectionName, '001');
        });

    });

    describe('#verifyTenderingAsset', () => {
        ...
        it('should return success message if hash provided matches the hash of the private data', async () => {
            ...
            ctx.stub.getPrivateDataHash.withArgs(collectionName, '001').resolves(Buffer.from(hashToVerify, 'hex'));
            const result = await contract.verifyTenderingAsset(ctx, mspid, '001', '{"privateValue":"125"}');
        ...
        it('should throw an error if hash provided does not match the hash of the private data', async () => {
            ...
            const result = await contract.verifyTenderingAsset(ctx, mspid, '001', 'someObject');
        ...
        it('should throw an error when user tries to verify an asset that doesnt exist', async () => {
            ...
            await contract.verifyTenderingAsset(ctx, mspid, '001', 'someObject').should.be.rejectedWith('No private data hash with the key: 001');
        });
    });
});
